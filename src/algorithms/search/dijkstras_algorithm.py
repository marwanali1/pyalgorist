...
def find_cheapest_route(start_city: City, end_city: City) -> list[str]:
    ...
    while discovered_cities:
        ...
        for adj_city in curr_city.routes:
            ...
            if (adj_city.name not in visited_cities) and (adj_city not in discovered_cities):
                discovered_cities.append(adj_city)
            
            price_through_curr_city = cheapest_prices[curr_city.name] + price_to_adj_city

            if (adj_city.name not in cheapest_prices) or price_through_curr_city < cheapest_prices[adj_city.name]:
